...
@section('title', 'Kategori Prasarana')
@section('page-title', 'Kategori Prasarana')
@section('page-subtitle', 'Kelola kategori untuk pengelompokan prasarana')
...
<div class="page-content page-content--data">
    ...
    {{-- Data Control --}}
    <section class="data-control" aria-label="Kontrol Data Kategori Prasarana">
        <form method="GET" action="{{ route('kategori-prasarana.index') }}" class="data-control__search" style="display:flex;align-items:center;width:100%;gap:12px;">
            <div style="flex:1;min-width:220px;">
                <x-input.text
                    type="search"
                    name="search"
                    :value="$filters['search'] ?? ''"
                    placeholder="Cari kategori prasarana..."
        ...
        <div class="data-control__actions">
            <a href="{{ route('kategori-prasarana.create') }}" class="c-button c-button--primary c-button--with-icon data-control__action" style="width:auto;text-decoration:none;">
    ...
    {{-- Data Table --}}
    <section class="data-table" aria-label="Tabel Kategori Prasarana">
            ...
            <table class="data-table__table">
                <x-table.head class="data-table__head">
                    <tr class="data-table__row">
                        ...
                        <x-table.th class="data-table__cell data-table__cell--meta">Jumlah Prasarana</x-table.th>
                ...
                <x-table.body class="data-table__body">
                    @forelse($kategoris as $index => $kategori)
                        <tr class="data-table__row" data-href="{{ route('kategori-prasarana.show', $kategori) }}" style="cursor:pointer;">
                            ...
                            <x-table.td class="data-table__cell">
                                <small style="color: var(--text-muted);">{{ $kategori->description ?? '-' }}</small>
                            </x-table.td>
                            <x-table.td class="data-table__cell data-table__cell--meta">
                                <x-badge variant="default" size="sm" rounded>
                                    {{ $kategori->prasarana_count ?? $kategori->prasarana()->count() }} prasarana
                                </x-badge>
                            </x-table.td>
                            <x-table.td class="data-table__cell data-table__cell--action">
                                <div style="display:flex;gap:8px;justify-content:flex-end;">
                                    <a href="{{ route('kategori-prasarana.edit', $kategori) }}" style="text-decoration:none;" class="js-row-action">
                                    ...
                                    <x-button
                                        ...
                                        size="sm"
                                        class="js-row-action"
                                        onclick="document.getElementById('deleteKategoriPrasaranaModal-{{ $kategori->id }}').open()"
                                    >
                                        Hapus
                                    </x-button>
                                </div>
                        ...
                        <x-modal id="deleteKategoriPrasaranaModal-{{ $kategori->id }}" title="Hapus Kategori" size="sm">
                            <form
                                id="delete-kategori-prasarana-form-{{ $kategori->id }}"
                                action="{{ route('kategori-prasarana.destroy', $kategori) }}"
                                method="POST"
                            ...
                            >
                                ...
                                <p style="font-size:0.9rem;color:var(--text-muted);">
                                    Yakin ingin menghapus kategori <strong>{{ $kategori->name }}</strong>? Tindakan ini tidak dapat dibatalkan.
                                </p>
                            </form>
                            ...
                            <x-slot:footer>
                                ...
                                <x-button
                                    type="submit"
                                    variant="danger"
                                    form="delete-kategori-prasarana-form-{{ $kategori->id }}"
                                >
                                    Hapus
                                </x-button>
                            </x-slot:footer>
                    ...
                    @empty
                        <tr class="data-table__row">
                            <x-table.td colspan="5" class="data-table__cell" align="center" style="padding:40px;color:var(--text-muted);">
                                Belum ada data kategori prasarana
                            </x-table.td>
                        </tr>
                    @endforelse
                </x-table.body>
