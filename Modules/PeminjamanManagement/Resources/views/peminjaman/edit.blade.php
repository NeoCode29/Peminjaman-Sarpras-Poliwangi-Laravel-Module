...
@section('page-subtitle', 'Perbarui pengajuan peminjaman')
...
<div class="page-content">
    ...
    <form action="{{ route('peminjaman.update', $peminjaman) }}" method="POST" enctype="multipart/form-data" class="form-section">
        @csrf
        @method('PUT')

        @php
            $formEventName = old('event_name', $peminjaman->event_name);
            $formStartDate = old('start_date', optional($peminjaman->start_date)->format('Y-m-d'));
            $formEndDate = old('end_date', optional($peminjaman->end_date)->format('Y-m-d'));
            ...
            $formLoanType = old('loan_type', $peminjaman->loan_type ?? 'prasarana');
        @endphp

        <x-form-group
            title="Informasi Kegiatan"
            description="Perbarui informasi dasar kegiatan."
                ...
                <div class="form-section__grid">
                            ...
                            <div style="display:flex;flex-wrap:wrap;gap:12px;margin-top:8px;">
                                <label style="display:flex;align-items:center;gap:6px;cursor:pointer;">
                                    <input type="radio" name="loan_type" value="prasarana"
                                        {{ $formLoanType === 'prasarana' ? 'checked' : '' }}
                    ...
                    @if(auth()->user()->user_type === 'mahasiswa')
                        <div class="form-field form-field--full">
                            ...
                            >
                                @foreach($ukms as $ukm)
                                    <option value="{{ $ukm->id }}" {{ (int)$formUkmId === $ukm->id ? 'selected' : '' }}>
                                        {{ $ukm->nama }}
                                    </option>
                                @endforeach
                            </x-input.select>
                        ...
                        <div class="form-field form-field--full" id="prasarana_container" style="{{ $formJenisLokasi === 'custom' ? 'display:none;' : '' }}">
                            ...
                            >
                                @foreach($prasarana as $item)
                                    <option value="{{ $item->id }}" {{ (int)$formPrasaranaId === $item->id ? 'selected' : '' }}>
        ...
        <x-form-group
            title="Lampiran"
            description="Perbarui surat pengajuan jika diperlukan."
                ...
                <div class="form-section__grid">
                    <div class="form-field">
                        <x-input.file
                            ...
                            id="surat"
                            accept=".pdf,image/*"
                            :helper="'Opsional. Kosongkan jika tidak diganti.'"
                        />
                        @error('surat')
                            <p class="c-input__helper is-invalid">{{ $message }}</p>
                        @enderror
                        ...
                        @if($peminjaman->surat_url)
                            <p class="c-input__helper" style="margin-top:8px;">
                                Surat saat ini:
                                <a href="{{ $peminjaman->surat_url }}" target="_blank" style="text-decoration:underline;">Lihat Surat</a>
                            </p>
                        @endif
                    </div>
                </div>
        ...
        <div class="form-section__actions form-section__actions--footer">
            <a href="{{ route('peminjaman.show', $peminjaman) }}" style="text-decoration: none;">
