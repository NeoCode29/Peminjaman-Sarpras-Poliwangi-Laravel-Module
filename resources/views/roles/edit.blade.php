...
@section('page-title', 'Edit Role: ' . ($role->name ?? 'Role'))
@section('page-subtitle', 'Ubah informasi dan permission role')
...
<div class="page-content">
    ...
    <form action="{{ route('role-management.update', $role->id) }}" method="POST" class="form-section">
        @csrf
        @method('PUT')

        <x-form-group
            title="Informasi Role"
            description="Perbarui informasi dasar role dan atur permission yang terkait."
            icon="heroicon-o-key"
        >
            <x-form-section>
                <div class="form-section__grid form-section__grid--single">
                    ...
                    @if(\App\Constants\ProtectedRoles::isProtected($role->name))
                        <small style="color: var(--text-muted); font-size: 0.85rem; margin-top: 4px; display: block;">
                            <strong>Catatan:</strong> Nama role ini tidak dapat diubah karena merupakan role sistem.
                        </small>
                    @endif
